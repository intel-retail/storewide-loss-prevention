...
if [ -f "${STREAM_DENSITY_SCRIPT}" ]; then
    python3 "${STREAM_DENSITY_SCRIPT}" generate \
        "${DLSTREAMER_TEMPLATE}" "${DLSTREAMER_CONFIG}" "${CAMERAS_COMPOSE}" \
        "${CAMERA_NAME}" "${VIDEO_FILE}" "${STREAM_DENSITY}"
else
    echo "  stream_density.py not found — generating single pipeline/camera"
    # Single pipeline: just replace {{CAMERA_NAME}} in template
    sed "s/{{CAMERA_NAME}}/${CAMERA_NAME}/g" "${DLSTREAMER_TEMPLATE}" > "${DLSTREAMER_CONFIG}"
    # Single camera compose
    cat > "${CAMERAS_COMPOSE}" <<CAMEOF
# Auto-generated by init.sh — do not edit
services:
  lp-cams:
    image: linuxserver/ffmpeg:version-8.0-cli
    command: -nostdin -re -stream_loop -1 -i /workspace/media/${VIDEO_FILE} -c:v copy -an -f rtsp -rtsp_transport tcp rtsp://mediaserver:8554/${CAMERA_NAME}
    volumes:
