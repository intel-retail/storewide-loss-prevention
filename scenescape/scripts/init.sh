...
AI_KEYS_REGEX='^(VLM_ENABLED|VLM_MODEL_NAME|VLM_PRECISION|TARGET_DEVICE|YOLO_MODEL_NAME|DETECT_MODEL|REID_MODEL|DETECT_DEVICE|REID_DEVICE|MODEL_PRECISION|SCENESCAPE_VERSION|SCENESCAPE_REGISTRY|DLSTREAMER_VERSION|RESULTS_PATH)='
...
# Auto-select pipeline template based on model type
# YOLO models need VA memory decode path (GPU template)
# OpenVINO models use software decode (CPU template) and can run on any device
DETECT_DEVICE="${DETECT_DEVICE:-GPU}"
REID_DEVICE="${REID_DEVICE:-CPU}"
DETECT_MODEL="${DETECT_MODEL:-yolo11s}"

if [[ "${DETECT_MODEL}" == yolo* ]]; then
    DLSTREAMER_TEMPLATE="${APP_DIR}/configs/pipeline-config-gpu.json"
    echo "  Pipeline: GPU decode (decodebin3 + VA memory + va-surface-sharing)"
else
    DLSTREAMER_TEMPLATE="${APP_DIR}/configs/pipeline-config-cpu.json"
    echo "  Pipeline: SW decode (avdec_h264)"
fi

# Fall back to generic template if device-specific one doesn't exist
