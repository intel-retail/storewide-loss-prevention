...
DIMENSIONS = int(os.getenv("REID_DIMENSIONS", "1280"))
K_NEIGHBORS = 1
...
class VDMSDatabase(ReIDDatabase):
  ...
  def addEntry(self, uuid, rvid, object_type, reid_vectors, set_name=SCHEMA_NAME, **metadata):
    ...
    for reid_vector in reid_vectors:
      ...
      if vec_array.shape[0] != DIMENSIONS:
        log.warning(f"addEntry: Expected vector shape ({DIMENSIONS},) but got {vec_array.shape}, skipping this vector")
        continue
