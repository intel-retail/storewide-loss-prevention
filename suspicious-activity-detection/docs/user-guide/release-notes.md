...
- Behavioral analysis service with YOLO pose estimation and VLM-based concealment
  detection using Intel® OpenVINO™ Model Server
...
| ------------------------------------ | ----------------------------- | ------------------------- |
| `yolov8s`                            | Person detection (DLStreamer) | Person bounding boxes     |
...
- The `fire_once_per: session` dedup means a repeated-visit alert won't re-fire even if
  the person continues visiting the zone after the threshold is crossed.
